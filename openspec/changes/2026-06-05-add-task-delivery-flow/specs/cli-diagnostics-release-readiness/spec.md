...
- **AND** if the final child JSONL event is emitted without a trailing newline, diagnostics flushes that final buffered event when the child process exits
- **AND** child-derived public fields and progress identifiers such as task id, baseline id, tool name, tool status, repair reason, and outcome are emitted only when they are short safe identifiers; unsafe values fall back to neutral public labels
...
- **中文** 当 `deepseek diagnostics evaluate --execute-task <id> --output text` 运行 DeepSeek CLI 子进程，且子进程在退出前输出 JSONL task events 时，diagnostics 可以为 tool intent、tool result、repair started、repair stopped、loop completed 与 loop failed 等 allowlist child events 流式输出有界公开进度行；如果最后一个 child JSONL event 没有 trailing newline，diagnostics 必须在子进程退出时 flush 该最终 buffered event；task id、baseline id、tool name、tool status、repair reason 与 outcome 等公开进度标识符只允许输出短安全标识符，unsafe values 必须回退为中性的公开标签；progress streaming 必须忽略 raw model deltas、provider raw chain-of-thought、raw stdout/stderr lines、raw model request/response bodies、raw secrets、environment credentials 与未识别的 child payloads。
